...
const AddressModal: React.FC<TAddressModal> = ({ handleModal }) => {
  const [isInputCountyChange, setIsInputCountyChange] = useState(false)
  const [isInputAddressChange, setIsInputAddressChange] = useState(false)
  const [inputCountryIsReady, setInputCountryIsReady] = useState(false)
  const [inputAddressIsReady, setInputAddressIsReady] = useState(false)

  const [inputValue, setInputValue] = useState<TInputData>({ country: '', address: '' })

  const handleInputCountyClick = () => {
    setIsInputCountyChange(prev => !prev)
    setInputCountryIsReady(false)
  }

  const handleInputAddressClick = () => {
    setIsInputAddressChange(prev => !prev)
  }

  const handleInputCountryClear = e => {
    e.preventDefault()
    setInputCountryIsReady(false)
    setInputValue(prev => ({ ...prev, country: '' }))
  }

  const handleInputAddressClear = e => {
    e.preventDefault()
  ...
  const handleKeyDownCountry = e => {
    if (e.key === 'Enter') {
      e.target.blur()
      setInputCountryIsReady(prev => !prev)
    }
  }
  ...
  const handleKeyDownAddress = e => {
    if (e.key === 'Enter') {
      e.target.blur()
      setInputAddressIsReady(prev => !prev)
    }
  }
        ...
        <div className={styles.AddressCreation_root}>
          <div className={styles.AddressCreation_map}>
            <div className={styles.MapContainer_container}>
              <YMap
                data={inputValue}
                inputCountryIsReady={inputCountryIsReady}
                setInputValue={setInputValue}
                inputValue={inputValue}
                inputAddressIsReady={inputAddressIsReady}
              />
            </div>
          </div>
          <div className={styles.AddressCreation_info}>
            <span className={styles.AddressCreation_text}>Добавить адрес</span>
            <div className={styles.AddressSuggest_root}>
              ...
              <div className={styles.Suggest_root}>
                <div className={styles.textInputContainer}>
                  <input
                    style={{
                      paddingLeft: `${isInputCountyChange || inputValue.country.length != 0 ? '0' : ''}`,
                    }}
                    ...
                    autoCorrect="off"
                    autoComplete="off"
                    onKeyDown={e => handleKeyDownCountry(e)}
                    placeholder={`${isInputCountyChange ? '' : 'Город'}`}
                  />
                  <div
                    style={{
                      opacity: `${isInputCountyChange || inputValue.country.length != 0 ? '0' : '1'}`,
                    }}
                  ...
                  <div
                    ...
                    <button onClick={e => handleInputCountryClear(e)}>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                  ...
                  <span
                    style={{
                      display: `${isInputCountyChange || inputValue.country.length != 0 ? 'block' : ''}`,
                    }}>
                    Город
                  </span>
                </div>
              ...
              {inputCountryIsReady && (
                <div className={styles.Suggest_root}>
                  <div className={styles.textInputContainer}>
                    <input
                      style={{
                        paddingLeft: `${isInputAddressChange || inputValue.address.length != 0 ? '0' : ''}`,
                      }}
                      onFocus={handleInputAddressClick}
                      onBlur={handleInputAddressClick}
                      onChange={e => handleInputChange(e)}
                      onKeyDown={e => handleKeyDownAddress(e)}
                      className={styles.textInput}
                      type="text"
                      id="address"
                      ...
                      autoCorrect="off"
                      autoComplete="off"
                      placeholder={`${isInputAddressChange ? '' : 'Улица и дом'}`}
                    />
                    <div
                      style={{
                        opacity: `${isInputAddressChange || inputValue.address.length != 0 ? '0' : '1'}`,
                      }}
                      className={styles.inputIcon}>
                      <svg
                        width="16"
                    ...
                    <div
                      style={{ opacity: `${isInputAddressChange ? '1' : '0'}` }}
                      className={styles.inputCrossIcon}>
                      <button onClick={e => handleInputAddressClear(e)}>
                        <svg
                          width="16"
                          height="16"
                          viewBox="0 0 16 16"
                    ...
                    <span
                      style={{
                        display: `${isInputAddressChange || inputValue.address.length != 0 ? 'block' : ''}`,
                      }}>
                      Улица и дом
                    </span>
                  </div>
            ...
            {inputValue.address.length != 0 && inputValue.address.length != 0 && (
              <div className={styles.AddressCreation_button}>
                <button onClick={handleCreateAddress} className={styles.AddressCreation_button__control}>
                  <span className={styles.AddressCreation_button__content}>
                    <span className={styles.AddressCreation_button__center}>
                      <span className={styles.AddressCreation_button__text}>Да, все верно</span>
                    </span>
                  </span>
                </button>
              </div>
